...
object Sequences: // Essentially, generic linkedlists
  ...
  object Sequence:
    ...
    def zip[A, B](first: Sequence[A], second: Sequence[B]): Sequence[(A, B)] = (first, second) match
      case (Cons(h, t), Cons(h1, t1)) => Cons((h, h1), zip(t, t1))
      case _ => Nil()
    ...
    def min(s: Sequence[Int]): Optional[Int] = s match
      case Cons(h, t) => t match
        case Cons(h1, t1) if h1 < h => min(Cons(h1, t1))
        case Cons(_, t1) => min(Cons(h, t1))
        case _ => Just(h)
      case _ => Empty()
    ...
    def distinct[A](s: Sequence[A]): Sequence[A] =
      @tailrec
      def _distinct(seq: Sequence[A], acc: Sequence[A]): Sequence[A] = seq match
        case Cons(h, t) => _distinct(t, if (contains(acc)(h)) acc else Cons(h, acc))
        case _ => reverse(acc)
      _distinct(s, Nil())
...
@main def trySequences =
  ...
  println(Sequence.sum(sequence)) // 60
