...
class SchoolTest:
  ...
  @Test def testTeacher(): Unit =
    assertEquals("Viroli", teacher("Viroli"))
  ...
  @Test def testCoursesOfATeacher(): Unit =
    ...
    assertEquals(Cons("Math", Nil()), school.coursesOfATeacher(teacher("John")))
    assertEquals(Cons("Math", Cons("Italian", Nil())), school3.coursesOfATeacher(teacher("John")))
