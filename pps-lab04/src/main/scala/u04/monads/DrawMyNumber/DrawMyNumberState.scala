...
object DrawMyNumberStateImpl extends DrawMyNumberState:
  ...
  override def initialState(topBound: Int, attempts: Int): DrawMyNumber =
    (new Random().nextInt(topBound), attempts, (topBound, attempts))

  override def getTopBoundAndAttempts: State[DrawMyNumber, (Int, Int)] = State(i => (i, i._3))

  override def nop(): State[DrawMyNumber, Unit] = State(i => (i, ()))

  override def guess(x: Int): State[DrawMyNumber, (Result, Int)] =
